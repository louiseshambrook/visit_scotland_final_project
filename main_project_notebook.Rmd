...
accomodation %>%
  filter(location != "Other") %>%
  group_by(location, accommodation_type, occupancy_type) %>%
  summarise(popular_accomodation = sum(value)/100) %>%
  ggplot(aes(x = location, y = popular_accomodation, fill = "blue")) +
...
Based on my initial analysis, I considered whether B&B could be increasing in popularity in urban areas (this appeared to be the trend). However, considering the breakdown in data in occupancy_type, this makes the data more complex to analyse and the trend less prominent.
...
accomodation %>%
  filter(location == "Urban area" & occupancy_type != "Pitch Occupancy" &
           occupancy_type != "Unit Occupancy" & accommodation_type != "Hostels") %>%
  group_by(location, accommodation_type, occupancy_type) %>%
  summarise(popular_accomodation = sum(value)/100) 
```
