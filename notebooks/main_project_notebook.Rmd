...
accomodation %>%
  ...
  summarise(popular_accomodation = sum(value)) %>%
  ggplot(aes(x = accommodation_type, y = popular_accomodation)) +
  geom_col()
...
```{r}
accomodation %>%
  filter(location != "Other" & year == 2019 & location == "Urban area") %>%
  group_by(location, accommodation_type) %>%
  summarise(popular_accomodation = sum(value)) %>%
  arrange(desc(popular_accomodation)) %>%
  ggplot(aes(x = accommodation_type, y = popular_accomodation)) +
...
activities %>%
  ...
  group_by(activity_type) %>%
  summarise(highest_income = sum(value)) %>%
  ggplot(aes(x = activity_type, y = highest_income)) +
  geom_col() +
  coord_flip()
...
So, with all of that information - who are the people visiting Scotland?


These are difficult to compare, as most of the questions/options are binary (married/not), whereas age has multiple options, so they can't really be fairly compared.
```{r}
# Who spends the most?
...
demographics %>%
  ...
  group_by(marital_status) %>%
  summarise(highest_spend = sum(value)) %>%
  ggplot(aes(x = marital_status, y = highest_spend)) +
  geom_col()

```
2013 - 2019, sum of spend in millions.

Note: I should really change this to a facet_wrap of the sum of all of the variables, or something.

Same as above, not really possible to compare.
```{r}
# Who visits the most?
demographics %>%
  filter(breakdown_of_domestic_tourism == "Visits" & children != "All") %>%
  group_by(children) %>%
  summarise(most_visits = sum(value)) %>%
  ggplot(aes(x = children, y = most_visits)) +
  geom_col()

```
2013-2019, sum of visits, in millions. 
...
demographics %>%
  ...
  group_by(age) %>%
  summarise(most_visits = sum(value)) %>%
  ggplot(aes(x = age, y = most_visits)) +
  geom_col()


```
While the two above have a skewed perspective, they do give insights into the data.
The other key demographic to focus on, is age. As reflected in the graph (visits, sum millions), it is the younger generations that are visiting Scotland.
...
regional_domestic %>%
  filter(breakdown_of_domestic_tourism != "Expenditure") %>%
  ggplot(aes(x = breakdown_of_domestic_tourism, y = value, fill = region_of_residence)) +
  geom_col(position = "dodge")


```
These two graphs kinda answer everything, but can also be a bit confusing to look at. I'll break them down a bit more.
...
regional_domestic %>%
  select(start_year, unit, value, region_of_residence, breakdown_of_domestic_tourism) %>%
  filter(breakdown_of_domestic_tourism == "Expenditure") %>%
  ggplot(aes(x = region_of_residence, y = value)) +
  geom_col()
...
```
These are answering "where are visitors from".

```{r}
# answering Q's:
# How do they differ in the money they spend?
...
Comparing all the datasets for change over time
...
regional_domestic %>%
  ...
  group_by(years) %>%
  summarise(total_year = sum(value)) %>%
  ggplot(aes(x = years, y = total_year)) +
  geom_col()

# this is for nights and visits
```
```{r}
regional_domestic %>%
  select(-visit_night_unit) %>%
  group_by(years) %>%
  summarise(total_year = sum(value)) %>%
  ggplot(aes(x = years, y = total_year)) +
  geom_col()
...
The picture over time, is generally increasing until 2017 - and then declining.
