...
accomodation %>%
  filter(location != "All") %>%
  group_by(location, accommodation_type_and_occupancy) %>%
  summarise(popular_accomodation = sum(value)) %>%
  arrange(desc(popular_accomodation))

```
To do this analysis properly, I need to clean/wrangle the set. I won't do this in the initial cleaning, I'll do it separately.
