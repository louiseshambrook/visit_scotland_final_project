...
```{r}
# to undo it truly, I have to read in the raw data.

location_redone <- read_csv("raw_data/tourism_day_visits_location.csv") %>%
  clean_names() %>%
  ...
  mutate(type_of_location_visited = 
           case_when(type_of_location_visited == "City/large town"
                     ...
                     ~ "Rural countryside",
                     type_of_location_visited == "Seaside - beach" ~ "Seaside - beach",
                     type_of_location_visited == "Seaside - other" ~ "Seaside - other",
...
location_redone <- location_redone %>%
  filter(breakdown_of_domestic_tourism == "Expenditure") %>%
  arrange(year)
...
location_redone <- location_redone %>%
  mutate(type_of_location_visited = 
           case_when(type_of_location_visited == "City/large town"
                     ...
                     type_of_location_visited == "Small town" ~ "Small town",
                     type_of_location_visited == "Village" ~ "Village",
                     type_of_location_visited == "Other" ~ "Other")) %>%
...
joined_activities_location <- inner_join(activities_redone, location_redone, by = "year")

joined_activities_location <- joined_activities_location %>%
  relocate(total_sum_location, .before = type_of_location_visited)

joined_activities_location %>%
  group_by(type_of_location_visited) %>%
  mutate(sum_per_activity_per_location = total_sum_activity + total_sum_location)
